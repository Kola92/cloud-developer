...
router.get("/:id", async (req: Request, res: Response) => {
  // Get an ID parameter off the url
  const { id } = req.params;

  // Query the feed by the ID
...
router.patch("/:id", requireAuth, async (req: Request, res: Response) => {
  ...
  // Get the caption payload from the request body
  const { caption } = req.body;

  // Query the feed by the ID
  const item = await FeedItem.findByPk(id);

  // If the item is found, update it and return it in the response
  if (item) {
